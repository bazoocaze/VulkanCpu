...
namespace Examples.vulkan_tutorial.com.Ex09_LoadingModels
{
	...
	public class HelloTriangleApplication09
	{
		public const int WIDTH = 800;
		public const int HEIGHT = 600;

		// public const string MODEL_PATH = "vulkan-tutorial.com/models/chalet.obj";
		// public const string TEXTURE_PATH = "vulkan-tutorial.com/textures/chalet.jpg";
		public const string MODEL_PATH = "c:/dados/temp/chalet.obj";
		public const string TEXTURE_PATH = "c:/dados/temp/chalet.jpg";
		...
		{
			var app = (HelloTriangleApplication09)GLFW.glfwGetWindowUserPointer(window);
		...
		private void createImageViews()
		{
			...
			{
				swapChainImageViews[i] = createImageView(swapChainImages[i], swapChainImageFormat, VkImageAspectFlagBits.VK_IMAGE_ASPECT_COLOR_BIT);
			}
		}
		...
		{
			var vertShaderModule = createShaderModule(typeof(Shader_Vert_09));
			var fragShaderModule = createShaderModule(typeof(Shader_Frag_09));
		...
		private void createTextureImage()
		{
			...
			byte[] pixels = STBI.stbi_load(Util.FindResource(TEXTURE_PATH), out texWidth, out texHeight, out texChannels, PixelFormat.Format32bppRgb);
		...
		private void loadModel()
		{
			...
			List<tinyobj.material_t> materials = new List<tinyobj.material_t>();
			string err;

			if (!tinyobj.LoadObj(out attrib, shapes, materials, out err, Util.FindResource(MODEL_PATH)))
			{
			...
			{
				foreach (var index in shape.mesh.indices)
				{
					...
					vertex.textCoord = new vec2(
						attrib.texcoords[2 * index.texcoord_index + 0],
						1.0f - attrib.texcoords[2 * index.texcoord_index + 1]);

					vertex.color = new vec3(1.0f, 1.0f, 1.0f);
		...
		{
			int bufferSize = Marshal.SizeOf(vertices[0]) * vertices.Count;
			...
			MemoryCopyHelper.Copy(vertices.ToArray(), data, 0, bufferSize);
			Vulkan.vkUnmapMemory(device, stagingBufferMemory);
		...
		{
			int bufferSize = Marshal.SizeOf(indices[0]) * indices.Count;
			...
			byte[] data;
			Vulkan.vkMapMemory(device, stagingBufferMemory, 0, bufferSize, 0, out data);
			MemoryCopyHelper.Copy(indices.ToArray(), data, 0, bufferSize);
			Vulkan.vkUnmapMemory(device, stagingBufferMemory);
		...
		private void createCommandBuffers()
		{
			...
			for (int i = 0; i < commandBuffersCount; i++)
			{
				...
				Vulkan.vkCmdBindIndexBuffer(commandBuffers[i], indexBuffer, 0, VkIndexType.VK_INDEX_TYPE_UINT32);
				...
				Vulkan.vkCmdDrawIndexed(commandBuffers[i], indices.Count, 1, 0, 0, 0);
		...
		private void updateUniformBuffer()
		{
			...
			ubo.model = mat4.Rotate(time * glm.Radians(90.0f / 100f), new vec3(0.0f, 0.0f, 1.0f));
